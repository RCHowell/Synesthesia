...
void makeSquare(char *, Image, int, int, int);
void checkerFilter(char *, Image, long, int);
...
int main (int argc, char * argv[]) {
    ...
    checkerFilter(buffer, image, lSize, filterLevel);
...
void checkerFilter(char * buffer, Image img, long l, int n) {
  ...
  int c = 0;
  while (c < 17) {
    makeSquare(buffer, img, i, size, ((x%2)==0));
    // increment i to where the next square needs to be started
    i += size;
    if (x > sperr * row) {
      printf("No. of Squares: %d\n", x);
      row++;
      i = size * sperr * row * x;
...
void makeSquare(char * buffer, Image img, int offset, int size, int isEven) {
  ...
  for (i = 0; i < size; i += 3) {
    for (j = 0; j < size; j += 3) {
      ...
      if (z < img.width * img.height * 3) {
        if   (isEven){ buffer[z] = 255; buffer[z+1] = 255; buffer[z+2] = 255;}
        else         { buffer[z] = 0;   buffer[z+1] =   0; buffer[z+2] = 0;}
      }
    }
  }
}
...
// buffer is byte data
// length is the overall byte data length
// w is original sampling zone width
// h is original sampling zone height
// x is starting x value in the sampling section
// y is starting y value in the sampling section
int averageColor(char * buffer, long length, int w, int h, int x, int y) {
  int samples = w*h;
  int sum = 0;
  ...
  for (i = x + y*col; i < samples; i++) {
    if (x % (w-1) == 0) col++;
    if (i + w*col < length) sum += buffer[i + w*col];
  }
